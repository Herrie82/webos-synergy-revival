...
static void updateDownloadProgress(const td::td_api::file &file, PurpleXfer *xfer, TdAccountData &account)
{
    ...
    // webOS tdlib-1.8.x: downloaded_size_ is int53; keep the max in int53 then narrow.
    int32_t  downloadedSize = std::max((td::td_api::int53)0,
                                       file.local_ ? file.local_->downloaded_size_ : (td::td_api::int53)0);
