...
static void tgprpl_info_show (PurpleConnection *gc, const char *who)
{
    ...
    for (const td::td_api::user *user: users) {
        ...
        // webOS tdlib-1.8.x: user.username_ -> user.usernames_ (object_ptr<usernames>);
        // the primary handle is editable_username_ (fallback: first active username).
        if (user->usernames_) {
            const std::string &primaryUsername = !user->usernames_->editable_username_.empty()
                ? user->usernames_->editable_username_
                : (user->usernames_->active_usernames_.empty() ? std::string()
                   : user->usernames_->active_usernames_.front());
            if (!primaryUsername.empty())
                // TRANSLATOR: Buddy infobox, key
                purple_notify_user_info_add_pair(info, _("Username"), primaryUsername.c_str());
        }
