...
void PurpleTdClient::processAuthorizationState(td::td_api::AuthorizationState &authState)
{
    ...
    // webOS Teams port / tdlib 1.8.x: authorizationStateWaitEncryptionKey was removed;
    // the database encryption key is now passed inside setTdlibParameters.
...
bool PurpleTdClient::addProxy()
{
    ...
    if (!errorMessage.empty()) {
        ...
        // webOS tdlib-1.8.x: addProxy now takes a proxy object (server/port/type) + enable + comment
        auto proxyObj = td::td_api::make_object<td::td_api::proxy>();
        proxyObj->server_ = host;
        proxyObj->port_ = port;
        proxyObj->type_ = std::move(tdProxyType);
        auto addProxyReq = td::td_api::make_object<td::td_api::addProxy>(std::move(proxyObj), true, std::string());
        m_transceiver.sendQuery(std::move(addProxyReq), &PurpleTdClient::addProxyResponse);
        m_isProxyAdded = true;
    }
...
{
    // webOS tdlib-1.8.x: getProxies now returns addedProxies (array<addedProxy>)
...
{
    // webOS tdlib-1.8.x: proxies are now addedProxy entries; proxy itself no longer has an id_,
    // so match the just-added proxy by server/port to avoid removing it, and remove the rest.
    for (const auto &added: m_proxies->proxies_)
        if (added && added->proxy_ &&
            (!m_addedProxy || added->proxy_->server_ != m_addedProxy->server_
                           || added->proxy_->port_   != m_addedProxy->port_))
            m_transceiver.sendQuery(td::td_api::make_object<td::td_api::removeProxy>(added->id_), nullptr);
}
...
// webOS: no request UI ops -> prompt for the value through a "Telegram" IM
// conversation. The user's reply is captured in sendMessage() (tgprpl_send_im).
void PurpleTdClient::promptAuthInputViaChat(AuthInput type, const std::string &message)
{
    m_authInputPending = type;
    std::string prompt = message;
    // TRANSLATOR: appended to the auth prompt shown in the fallback "Telegram" chat.
    prompt += std::string("\n") + _("Reply to this chat with the value above to continue.");

    PurpleConversation *conv = purple_conversation_new(PURPLE_CONV_TYPE_IM, m_account, "Telegram");
    if (conv)
        purple_conversation_write(conv, "Telegram", prompt.c_str(),
                                  (PurpleMessageFlags)(PURPLE_MESSAGE_RECV | PURPLE_MESSAGE_SYSTEM),
                                  time(NULL));
}
...
void PurpleTdClient::requestAuthCode(const td::td_api::authenticationCodeInfo *codeInfo)
{
    ...
    if (!purple_request_input (purple_account_get_connection(m_account),
                               ...
                               this))
    {
        // webOS: request UI unavailable -> ask for the code via the "Telegram" chat.
        promptAuthInputViaChat(AuthInput::Code, message);
    }
}
...
void PurpleTdClient::requestPassword(const td::td_api::authorizationStateWaitPassword &pwInfo)
{
    ...
    if (!purple_request_input (purple_account_get_connection(m_account),
                               ...
                               this))
    {
        // webOS: request UI unavailable -> ask for the 2FA password via the "Telegram" chat.
        std::string pwMessage = _("Enter password for two-factor authentication");
        if (!hints.empty())
            pwMessage += std::string("\n") + hints;
        promptAuthInputViaChat(AuthInput::Password, pwMessage);
    }
}
...
void PurpleTdClient::registerUser()
{
    ...
    if (firstName.empty() && lastName.empty()) {
        if (!purple_request_input (purple_account_get_connection(m_account),
                                ...
                                this))
        {
            // webOS: request UI unavailable -> ask for the display name via the "Telegram" chat.
            promptAuthInputViaChat(AuthInput::DisplayName,
                _("New account is being created. Please enter your display name."));
        }
    } else
...
void PurpleTdClient::notifyAuthError(const td::td_api::object_ptr<td::td_api::Object> &response)
{
    ...
    // webOS tdlib-1.8.x: authorizationStateWaitEncryptionKey removed (db key folded into
    // setTdlibParameters); no dedicated encryption-key error state remains.
    default:
...
int PurpleTdClient::sendMessage(const char *buddyName, const char *message)
{
    ...
    // treat this outgoing message as that value instead of sending it to anyone.
    if (m_authInputPending != AuthInput::None) {
        // OTR/other plugins may inject non-code messages; ignore those.
        if (message && g_str_has_prefix(message, "?OTR"))
            return -1;
        char *stripped = purple_markup_strip_html(message ? message : "");
        g_strstrip(stripped);
        AuthInput type = m_authInputPending;
        m_authInputPending = AuthInput::None;
        switch (type) {
            case AuthInput::Code:        requestCodeEntered(this, stripped);  break;
            case AuthInput::Password:    passwordEntered(this, stripped);     break;
            case AuthInput::DisplayName: displayNameEntered(this, stripped);  break;
            default: break;
        }
        g_free(stripped);
        return 1;
...
void PurpleTdClient::handleUserChatAction(const td::td_api::updateChatAction &updateChatAction)
{
    ...
    if (chatUserId != getUserId(updateChatAction)) {
        purpleDebug("Got user action for private chat {} (with user {}) for another user {}", {
            ...
            std::to_string(getUserId(updateChatAction).value())
        });
    } else if (updateChatAction.action_) {
        if (updateChatAction.action_->get_id() == td::td_api::chatActionCancel::ID) {
            purpleDebug("User (id {}) stopped chat action", getUserId(updateChatAction).value());
            ...
            purpleDebug("User (id %d): treating chatActionStartPlayingGame as cancel",
                        getUserId(updateChatAction).value());
            showUserChatAction(getUserId(updateChatAction), false);
        } else {
            purpleDebug("User (id {}) started chat action (id {})", {
                std::to_string(getUserId(updateChatAction).value()), std::to_string(updateChatAction.action_->get_id())
            });
                                ...
                                const std::string &groupName)
{
    ...
    if (users.size() == 1)
        ...
        // webOS tdlib-1.8.x: importContacts.contacts_ is a vector of importedContact (not contact)
                                    ...
                                    const std::string &groupName)
{
    ...
    // webOS tdlib-1.8.x: addContact now takes (user_id, importedContact, share_phone_number)
    td::td_api::object_ptr<td::td_api::importedContact> contact =
