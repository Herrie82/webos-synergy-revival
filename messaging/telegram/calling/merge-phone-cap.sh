#!/bin/sh
# Runs ON THE DEVICE. Adds the PHONE capabilityProvider (com.palm.telegram.call) to the Telegram account's
...
# isolate the Telegram account object (one per line after splitting on _kind)
SIG=$(printf '%s' "$ACC" | sed 's/{"_kind"/\n{"_kind"/g' | grep '"templateId":"com.palm.telegram"')
[ -n "$SIG" ] || { echo "FAIL: no Telegram account found"; exit 1; }

if printf '%s' "$SIG" | grep -q 'com.palm.telegram.call'; then
  ...
  printf '%s' "$ARR" | grep -q 'com.palm.telegram.im' || { echo "FAIL: array missing .im provider"; exit 1; }
  printf '%s' "$ARR" | grep -q 'com.palm.telegram.contacts' || { echo "FAIL: array missing .contacts provider"; exit 1; }
  [ "${ARR%\}}" != "$ARR" ] || { echo "FAIL: providers array did not end cleanly (last char: $(printf '%s' "$ARR" | tail -c 1))"; exit 1; }
  PHONE='{"capability":"PHONE","id":"com.palm.telegram.call","alwaysOn":true,"loc_shortName":"Telegram","implementation":"palm://com.palm.telegram.call/","serviceName":"type_telegram"}'
...
# The plugin is already loaded; com.palm.telegram.call registers when Telegram (re)logs in via callLunaInit.
# Do a light role rescan and just report current status.
ls-control scan-services 2>/dev/null || true

echo "=== status ==="
ls-monitor -l 2>/dev/null | grep -i 'telegram.call' && echo "  ^ com.palm.telegram.call is registered" \
  || echo "  com.palm.telegram.call not on bus yet (registers on next Telegram login; a transport restart when the device is idle will force it)"
echo "=== done - PHONE cap is set; the call service will ring the Phone app on the next incoming Telegram call ==="
