...
// The FFT twiddles are constants that depend only on (n,sign); the original code recomputed cos/sin on
// every transform, which profiled at ~half of the whole MLow encode time. The MLow analysis calls the
// FFT at a handful of fixed sizes each frame, so after the first frame every angle is a table lookup.
// The angle is formed from the reduced index m in [0,n) (mathematically identical since cos/sin are
// 2*pi periodic, and numerically cleaner than the original float32 k*j product).
