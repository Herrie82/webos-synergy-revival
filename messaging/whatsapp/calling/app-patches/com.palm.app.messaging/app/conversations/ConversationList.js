enyo.kind({
		...
		{kind: "ApplicationEvents", onUnload: "windowUnloadHandler", onWindowHidden:"windowHiddenHandler"},
			...
			{kind: "VFlexBox", flex: 1, onclick:"disableKeyboardMannualMode", components: [
				{kind: "Toolbar", className:"enyo-toolbar-light conversation-header", layoutKind: "HFlexLayout", align: "center", components: [
					...
					// webOS Synergy Revival: one-tap voice call for call-capable 1:1 IM chats. Hidden by
					// default; shown per-conversation in setupNewChatThread(). Leverages the stock Phone
					// app (the Palm Way) via the WhatsApp VoIP transport. Add more services in updateCallButton().
					{name: "voipCallButton", kind: "IconButton", icon: "images/voip-call-icon.png", showing: false, onclick: "voipCall", className: "conversation-call-btn"},
				...
				{name: "phoneAccounts", kind: "Accounts.getAccounts", onGetAccounts_AccountsAvailable: "gotPhoneAccounts"},
	...
	create: function() {
		...
		// Discover which IM services can place calls, by PHONE capability (see gotPhoneAccounts).
		this._callableServices = {};
		this.$.phoneAccounts.getAccounts({capability: "PHONE"});
...
//todo: also need check multiple converstions (chatthreadId)
	...
	// Build the map of call-capable IM services from the account framework's PHONE capability
	// (the same signal CallSynergizer uses to discover transports): serviceName -> account templateId
	// (the dial transport). Keying off the capability - not a hard-coded name - means any IM account
	// granted the PHONE capability (WhatsApp today; Telegram/Signal later) lights up automatically with
	// zero code change. type_* filter keeps it to IM VoIP; cellular/SMS stay on the existing dial path.
	gotPhoneAccounts: function(inSender, inResponse){
		var map = {};
		var accts = (inResponse && inResponse.accounts) || [];
		for (var i = 0; i < accts.length; i++){
			var tid = accts[i].templateId, cps = accts[i].capabilityProviders || [];
			for (var j = 0; j < cps.length; j++){
				var sn = cps[j].serviceName;
				if (sn && sn.indexOf("type_") === 0){ map[sn] = tid; }
			}
		}
		this._callableServices = map;
		this.updateCallButton();
	},
	// Show the call button only for a 1:1 conversation whose IM service advertises PHONE capability.
	updateCallButton: function(){
		var svc = this.chatThread && this.chatThread.replyService;
		var callable = !!(svc && this._callableServices[svc]) && this.chatThread && !this.chatThread.groupChatId;
		this.$.voipCallButton.setShowing(!!callable);
	},
	// Place a voice call to the current 1:1 buddy through the stock Phone app, routed via that
	// service's own transport (the account templateId) - not a hard-coded one.
	voipCall: function(){
		var addr = this.chatThread && this.chatThread.replyAddress;
		var transport = this._callableServices[this.chatThread && this.chatThread.replyService];
		if (!addr || !transport) { return; }
		this.$.launchApp.call({id: "com.palm.app.phone", params: {address: addr, transport: transport, video: false}});
	},
