enyo.kind({
	...
	create: function() {
		...
		//call.contact.decorated(function(line) {
			...
			// When no real contact resolved, contactname is a bare phone number (VoIP addressFormatted
			// is the raw +E.164) -> format it. Real names aren't valid numbers, so they pass through.
			if (enyo.application.Utils.isValidNumber(contactname)) {
				contactname = enyo.application.Utils.FormatPhoneNumber(contactname) || contactname;
			}
