...
enyo.kind({
	...
	statics: {
		...
		generateId: function(type, phonecall) {
			...
			} else if ( remote.service === enyo.application.CallSynergizer.TRANSPORTS.TIL || remote.service === enyo.application.CallSynergizer.TRANSPORTS.VOIP ) {
				address = "_PHONE_" + remote.normalizedAddr;

			// or, if skype, just use that
			} else if ( remote.service === enyo.application.CallSynergizer.TRANSPORTS.VOIP ) {
