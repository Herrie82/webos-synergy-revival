...
static void discord_add_dm_buddy(DiscordAccount *da, const gchar *merged_username, DiscordUser *user);
static void discord_backfill_dm_history(DiscordAccount *da);
...
discord_handle_guild_member_update(DiscordAccount *da, guint64 guild_id, JsonObject *data) {
	...
	if (guild && user) {
		...
		/* webOS: DM partners (e.g. mazzinia, rad.be.me) are non-friends whose presence
		 * only ever arrives via guild member updates, not PRESENCE_UPDATE. discord_update_status
		 * updates the internal DiscordUser but never pushes to the buddy list, so these DM
		 * buddies always read offline on webOS even though Discord shows them online. If this
		 * user is a DM buddy, propagate the presence to the blist (mirrors discord_got_presences). */
		{
			JsonObject *presence = json_object_get_object_member(data, "presence");
			if (presence != NULL) {
				gchar *dm_name = discord_create_fullname(user);
				if (dm_name != NULL) {
					if (purple_blist_find_buddy(da->account, dm_name) != NULL) {
						const gchar *pstatus = json_object_get_string_member(presence, "status");
						if (pstatus != NULL) {
							purple_protocol_got_user_status(da->account, dm_name, pstatus, "message",
								user->game ? user->game : user->custom_status, NULL);
							purple_protocol_got_user_idle(da->account, dm_name, purple_strequal(pstatus, "idle"), 0);
						}
					}
					g_free(dm_name);
				}
	...
	if (purple_strequal(type, "PRESENCE_UPDATE")) {
		...
		if (channel_type == CHANNEL_DM) {
			...
			if (first_recipient != NULL) {
				DiscordUser *dm_user = discord_upsert_user(da->new_users, first_recipient);
				...
				if (combined_username != NULL) {
					...
					g_hash_table_replace(da->last_message_id_dm, g_strdup(channel_id), g_strdup(last_message_id));
					/* webOS: also add as a contact (see discord_add_dm_buddy) */
					discord_add_dm_buddy(da, combined_username, dm_user);
		...
		/* webOS: backfill recent DM history so opened DM threads aren't empty */
		discord_backfill_dm_history(da);
...
/* Add a 1:1 DM partner to the buddy list. Discord only sends "relationships"
 * (friends) for auto-buddy, but on webOS the Messaging/Contacts UI only surfaces
 * people who are actual buddies -> DM partners that aren't friends (e.g. someone
 * you message in a shared server) would never appear. Mirror the friend buddy-add
 * so every open 1:1 DM shows up as a contact. */
static void
discord_add_dm_buddy(DiscordAccount *da, const gchar *merged_username, DiscordUser *user)
{
	if (merged_username == NULL || *merged_username == '\0') {
		return;
	}
	...
	if (buddy == NULL) {
		PurpleGroup *buddy_group = discord_get_or_create_default_group();
		buddy = purple_buddy_new(da->account, merged_username, user ? user->name : NULL);
		purple_blist_add_buddy(buddy, NULL, buddy_group, NULL);
	}
	...
	for (i = len - 1; i >= 0; i--) {
		...
		if (room_type == 1) {
			...
			if (recipients == NULL) {
				...
				dm_user = discord_get_user(da, to_int(user_id));
				merged_username = discord_create_fullname(dm_user);

			} else {
				// Old API
				JsonObject *user = json_array_get_object_element(recipients, 0);
				dm_user = discord_upsert_user(da->new_users, user);
			...
			if (merged_username != NULL) {
				...
				/* webOS: surface the DM partner as a contact (not just an internal
				 * one_to_ones entry) so it appears in Messaging/Contacts. */
				discord_add_dm_buddy(da, merged_username, dm_user);
...
discord_buddy_guild(DiscordAccount *da, DiscordGuild *guild)
{
	...
	int dbg_total = 0, dbg_added = 0, dbg_hidden = 0;
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		DiscordChannel *channel = value;
		dbg_total++;
		...
		if (!discord_is_channel_visible(da, user, channel)) {
			dbg_hidden++;
			purple_debug_info("discord", "buddy_guild: guild '%s' HIDE channel '%s' (type %d)\n",
			                  guild->name ? guild->name : "?", channel->name ? channel->name : "?", (int)channel->type);
			continue;
		}
		dbg_added++;
	...
	purple_debug_info("discord", "buddy_guild: guild '%s' (id %" G_GUINT64_FORMAT ") channels=%d added=%d hidden=%d\n",
	                  guild->name ? guild->name : "?", guild->id, dbg_total, dbg_added, dbg_hidden);
}
...
/* webOS has no local message logs, so opening a DM shows an empty thread unless we
 * proactively backfill it. Fetch the most recent messages once per DM (persisted
 * "dm-backfilled-<id>" marker in accounts.xml so reconnects don't re-fetch/duplicate),
 * and only for DMs active within the last 30 days to bound API calls + pipeline load.
 * Requests are staggered to respect Discord's rate limits. */
static void
discord_backfill_dm_history(DiscordAccount *da)
{
	if (!purple_account_get_bool(da->account, "fetch-unread-on-start", TRUE))
		return;

	const gint64 recent_cutoff = (gint64) time(NULL) - (30 * 24 * 3600);
	GHashTableIter iter;
	gpointer key, value;
	guint delay = 0;

	g_hash_table_iter_init(&iter, da->one_to_ones);
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		const gchar *channel_id = key;

		const gchar *last_msg = g_hash_table_lookup(da->last_message_id_dm, channel_id);
		guint64 last_id = last_msg ? to_int(last_msg) : 0;
		if (last_id == 0)
			continue;
		if ((gint64) discord_time_from_snowflake(last_id) < recent_cutoff)
			continue; /* dead DM, skip */

		gchar *marker = g_strdup_printf("dm-backfilled-%s", channel_id);
		if (!purple_account_get_bool(da->account, marker, FALSE)) {
			purple_account_set_bool(da->account, marker, TRUE);
			gchar *url = g_strdup_printf("https://" DISCORD_API_SERVER "/api/" DISCORD_API_VERSION
			                             "/channels/%s/messages?limit=25", channel_id);
			discord_fetch_url_with_delay(da, url, NULL, discord_got_history_static, NULL, delay);
...
discord_got_read_states(DiscordAccount *da, JsonNode *node, gpointer user_data)
{
	...
	for (int i = len - 1; i >= 0; i--) {
		...
		if (channel) {
			...
			/* webOS: seed last_message_id_dm from the read state so discord_backfill_dm_history
			 * (runs right after got_read_states) will backfill DMs that are known only via a
			 * read_state entry, not READY's private_channels list (Discord trims that list, so
			 * partners like mazzinia/rab.be.me have no cached last_message_id and would be
			 * skipped by backfill's `last_id == 0 -> continue` guard, leaving empty history). */
			if (isDM && last_id != 0) {
				const gchar *have = g_hash_table_lookup(da->last_message_id_dm, channel);
				if (have == NULL || to_int(have) == 0)
					g_hash_table_replace(da->last_message_id_dm, g_strdup(channel), g_strdup(last_id_s));
			}
...
discord_login(PurpleAccount *account)
{
	...
	/* webOS Synergy: the legacy imlibpurple config-kind path is disabled, so the
	 * "token" account option can't be pre-seeded. The customUI stores the Discord
	 * auth token as the account PASSWORD (the universal credentials path). If no
	 * token option was set, use that password as the token. Discord blocks
	 * third-party email/password (captcha) anyway, so this is the working path.
	 * A special "QRLOGIN" sentinel password selects the QR / remote-auth flow. */
	gboolean discord_qr_login = purple_strequal(account_password, "QRLOGIN");
	/* webOS: da->token above is loaded from the persisted "token" account setting
	 * (accounts.xml). A QR approval saves it via purple_account_set_string, so on
	 * later reconnects we already have it here -> use it and skip QR (the original
	 * code went straight to QR whenever the password was the "QRLOGIN" sentinel,
	 * re-prompting every time). If the token is revoked, the 4004 handler clears
	 * it and the next connect falls back to QR. */
	if (!discord_qr_login && (!da->token || !*da->token) && account_password && *account_password) {
		g_free(da->token);
		da->token = g_strdup(account_password);
	}
	if (da->token && *da->token) {
		discord_start_socket(da);

	} else if (!discord_qr_login && account_password && *account_password) {
		JsonObject *data = json_object_new();
		gchar *str;
                                     ...
                                     G_GNUC_UNUSED gpointer user_data)
{
	...
	/* Persisted in accounts.xml; reused on reconnect so QR is one-time (see discord_login) */
				...
				if (ya->packet_code == 136) {
					...
					if (length_code > 0 && length_code <= 125) {
						...
						if (purple_ssl_read(conn, &error_buf, 2) == 2) {
							...
							if (error_code == 4004) {
								/* bad auth token, clear and reset (accounts.xml persists
								 * the cleared value, so the next connect re-QRs) */
