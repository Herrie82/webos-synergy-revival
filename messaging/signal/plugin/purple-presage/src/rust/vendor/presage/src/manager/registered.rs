...
impl<S: Store> Manager<S, Registered> {
    ...
    /// webOS: the cached identified (credentialed) PushService, so the purple-presage backend can
    /// GET /v2/calling/relays for the signal_media libnice agent's STUN/TURN config. The only local
    /// change to this vendored presage (see the [patch] in ../../Cargo.toml).
    pub fn webos_identified_push_service(&self) -> PushService {
