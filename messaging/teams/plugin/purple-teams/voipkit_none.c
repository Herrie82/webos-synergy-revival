...
//
// Rather than thread #ifdefs through teams_call_luna.c's call sequencing, the whole video bridge
// is swapped out at link time: build-teams.sh compiles this file instead of voipkit.cpp when
// TEAMS_VOIPKIT=0, and drops -lpalmgstskype. Everything else in the plugin -- messaging,
...
static void warn_once(const char *fn)
{
	if (s_warned)
		return;
	s_warned = 1;
	fprintf(stderr, "teams: %s called, but this build has no SkypeKit video bridge "
 ...
 * honest answer here and is the value teams_call_luna.c already handles: it logs and carries on
