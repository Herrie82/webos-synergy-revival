/*global OAuth2, DropboxApi, console */
 ...
 * Returns credentials in the shape the account DB stores and DropboxApi later reads,
...
ExchangeCodeCommandAssistant.prototype = {
	run: function (future) {
		...
		ex.then(this, function () {
			...
			var info = DropboxApi.getAccountInfo(common);
			info.then(this, function () {
				var me = {};
				try { me = info.result || {}; } catch (e2) { me = {}; }
				future.result = {
					returnValue: true,
					username:    me.email || "Dropbox",
					alias:       (me.name && me.name.display_name) || undefined,
