...
var UrlSchemes = {
	...
	urlSchemes: {
		...
		google: {
			...
			//Google moved CalDAV to apidata.googleusercontent.com; the old googleapis.com/caldav path
			//now 404s / SERVICE_DISABLEs. CardDAV stays on www.googleapis.com/carddav/v1. Both need a
			//Bearer token (OAuth) and the CalDAV/Contacts-CardDAV APIs enabled in the Cloud project.
			//calendar: the /EMAIL/ collection (trailing slash, NOT /user) PROPFIND-lists every calendar
			//with a caldav:calendar resourcetype, so resolveHome finds them directly. Pointing this at
			///user (the principal) makes calendar resolveHome fail and drops discovery into the generic
			//fallback, which then clobbers the (correctly resolved) CardDAV contact home.
