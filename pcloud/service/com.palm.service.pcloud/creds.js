...
/* creds.js - resolve/persist pCloud account credentials from the service side.
 *
 ...
 * app and lets the service persist changes straight back to the account.
 * Requires com.palm.service.pcloud in the template read+writePermissions (the accounts
 * service permission-checks readCredentials/writeCredentials against those lists).
 *
 * The stored `common` object carries the REGION HOST (common.apiHost, e.g. api.pcloud.com
 * or eapi.pcloud.com) alongside the access token, so PcloudApi always talks to the right
 * data region for this account.
 */
