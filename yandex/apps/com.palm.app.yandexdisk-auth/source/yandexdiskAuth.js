 ...
 *   4. we close the Atlas card, then hand the code to com.palm.service.yandexdisk
...
enyo.kind({
	name: "YandexdiskService",
	kind: "PalmService",
	service: "palm://com.palm.service.yandexdisk/"
});
...
enyo.kind({
	...
	redirectPrefix: "http://localhost/yandex/oauth2callback",
	oauthResultKey: "x_yandex_oauth_result",

	components: [
		{ kind: "Toolbar", className: "enyo-toolbar-light accounts-header", pack: "center", components: [
			{ kind: "Control", name: "title", content: "Yandex Disk" }
		]},
		{ name: "svc",        kind: "YandexdiskService", onFailure: "svcFailure" },
		...
		{ kind: "Scroller", flex: 1, components: [
			{ className: "box-center", components: [
				{ name: "status", className: "accounts-body-text", style: "padding:16px; line-height:1.4;",
					content: "Tap Sign In to open the Yandex sign-in page. After you approve access, you'll be returned here automatically." }
			]}
		]},
	...
	create: function () {
		...
		this.log("yandexdisk-auth: launch params " + enyo.json.stringify(this.params));
	},
	...
	performSignIn: function () {
		...
		this.setStatus("Opening Yandex sign-in…");
		this.$.svc.call({ state: "yandex" }, {
	...
	launchAtlas: function (inSender, inResponse) {
		...
		// Yandex redirect is never captured and the pref stays empty (=> "credentials
		// incorrect"). Close any running Atlas first, then launch fresh.
		this.log("yandexdisk-auth: probing for a running Atlas to close before launch");
	...
	atlasClosedThenLaunch: function (inSender, resp) {
		...
		for (var i = 0; i < apps.length; i++) {
			if (apps[i].id === "org.webosports.app.atlas" && apps[i].processid) {
				this.log("yandexdisk-auth: closing stale Atlas pid " + apps[i].processid);
	...
	doLaunchAtlas: function () {
		if (this.done) { return; }
		this.log("yandexdisk-auth: launching Atlas simple-mode (cold)");
		this.$.appLaunch.call({
	...
	finish: function (result) {
		this.done = true;
		this.stopPoll();
		this.log("yandexdisk-auth: sending result " + enyo.json.stringify(result));
