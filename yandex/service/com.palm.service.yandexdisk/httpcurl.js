...
/*
 * httpcurl.js - HTTPS transport for the Yandex Disk service, via the modern curl.
 *
 ...
 * cloud-api.yandex.net / oauth.yandex.com. This mirrors the Teams port's rule - "no
 * modern HTTPS in the webOS JS layer" - by shelling every Yandex request out to the
 * modern curl (curl 7.88.1 + OpenSSL 1.1.1, TLS 1.3) from the deployment-bundle. The
 * JS layer is pure orchestration; all TLS lives in curl.
 *
 * COPIED VERBATIM from the Dropbox/OneDrive connector (identical curl invocation) -
 * only base URLs differ, and those live in the API layer, not here. `follow` (-L) is
 * used by the Disk download/upload flow: /resources/download returns a signed href we
 * GET (which may 302 to a storage host), and /resources/upload returns a PUT href.
 *
 * SECURITY: for a public (PKCE-only) build there is no client_secret; for a
 * confidential build the client_secret is one of the --data-urlencode form fields
 * (briefly visible in `ps` argv). The refresh_token is the only durable secret; on
 * this single-user rooted device that is acceptable. TODO harden via `curl --config -`.
 */
...
var HttpCurl = {
	// request({ method, url, headers:{}, form:{k:v}, body:"", bearer:"", dataFile, outFile, follow }) -> Future
	...
	request: function (opts) {
		...
		if (opts.follow) {
			args.push("-L");   // Disk download href may 302-redirect to a storage host
		}
		// outFile: write the response BODY straight to a local file (Disk download).
		...
		if (opts.form) {
			...
			// Send a local file as the raw request body (Disk upload PUT). "@" makes curl
