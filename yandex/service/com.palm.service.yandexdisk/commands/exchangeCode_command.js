/*global OAuth2, YandexApi, console */
 ...
 * Returns credentials in the shape the account DB stores and YandexApi later reads,
 * PLUS a username: accounts/handlers/create.js does Assert.require(args.username) and
 * refuses to create the account without one, so we fetch the Yandex email here.
 */
...
ExchangeCodeCommandAssistant.prototype = {
	run: function (future) {
		...
		// the moment ex completes, before the .then below can wrap it - so Accounts gets no
		// `credentials` and stores nothing. Instead resolve `future` ourselves, wrapped.
		...
		ex.then(this, function () {
			...
			var info = YandexApi.getUserInfo(common);
			info.then(this, function () {
				var me = {};
				try { me = info.result || {}; } catch (e2) { me = {}; }
				future.result = {
					returnValue: true,
					username:    me.default_email || me.login || "Yandex Disk",
					alias:       me.real_name || me.display_name || undefined,
