 ...
 * client_id / redirect_uri / scope all live in the service config (single source of
 * truth), so the app never needs to know them.
 * Called by com.palm.app.yandexdisk-auth at the start of account creation.
 */
function GetAuthorizeUrlCommandAssistant() {}

GetAuthorizeUrlCommandAssistant.prototype = {
	allowedAppIds: ["com.palm.app.yandexdisk-auth"],
	...
	run: function (future) {
		...
		try {
			...
			var url = OAuth2.buildAuthorizeUrl(args.state || "yandex");
			future.result = {
				returnValue: true,
				url: url,
