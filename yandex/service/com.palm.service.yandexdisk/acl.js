...
var Acl = {
	...
	enforce: function (assistant, future) {
		...
		if (Acl.BRINGUP && caller === "") {
			console.log("yandexdisk: ACL BRINGUP - allowing anonymous shell caller (testing only)");
			return true;
		}
		for (var i = 0; i < allowed.length; i++) {
			if (Acl._match(allowed[i], caller)) { return true; }
		}
		console.log("yandexdisk: ACL denied caller='" + caller + "'; method allows " + allowed.join(","));
