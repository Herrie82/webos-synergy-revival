...
/*
 * BoxnetAuth - customUI account validator for the Box Synergy connector.
 *
 ...
 * Box OAuth consent page (no TLS 1.2/1.3, modern JS). So this app does NO
...
enyo.kind({
	name: "BoxnetService",
	kind: "PalmService",
	service: "palm://com.palm.service.boxnet/"
});
...
enyo.kind({
	...
	redirectPrefix: "http://localhost/boxnet/oauth2callback",
	oauthResultKey: "x_boxnet_oauth_result",

	components: [
		{ kind: "Toolbar", className: "enyo-toolbar-light accounts-header", pack: "center", components: [
			{ kind: "Control", name: "title", content: "Box" }
		]},
		{ name: "svc",        kind: "BoxnetService", onFailure: "svcFailure" },
		...
		{ kind: "Scroller", flex: 1, components: [
			{ className: "box-center", components: [
				{ name: "status", className: "accounts-body-text", style: "padding:16px; line-height:1.4;",
					content: "Tap Sign In to open the Box sign-in page. After you approve access, you'll be returned here automatically." }
			]}
		]},
	...
	create: function () {
		...
		this.log("boxnet-auth: launch params " + enyo.json.stringify(this.params));
	},
	...
	performSignIn: function () {
		...
		this.setStatus("Opening Box sign-in…");
		this.$.svc.call({ state: "boxnet" }, {
	...
	launchAtlas: function (inSender, inResponse) {
		...
		// Box redirect is never captured and the pref stays empty (=> "credentials
		// incorrect"). Close any running Atlas first, then launch fresh.
		this.log("boxnet-auth: probing for a running Atlas to close before launch");
	...
	atlasClosedThenLaunch: function (inSender, resp) {
		...
		for (var i = 0; i < apps.length; i++) {
			if (apps[i].id === "org.webosports.app.atlas" && apps[i].processid) {
				this.log("boxnet-auth: closing stale Atlas pid " + apps[i].processid);
	...
	doLaunchAtlas: function () {
		if (this.done) { return; }
		this.log("boxnet-auth: launching Atlas simple-mode (cold)");
		this.$.appLaunch.call({
	...
	finish: function (result) {
		this.done = true;
		this.stopPoll();
		this.log("boxnet-auth: sending result " + enyo.json.stringify(result));
