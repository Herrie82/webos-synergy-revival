...
/* oauth2.js - OAuth2 Authorization Code + PKCE for Box, mirroring the Dropbox connector.
 *
 ...
 * Box also supports a confidential secret - if Config.CLIENT_SECRET is set to a real
 * value it is included too (some Box app types require it); left as the placeholder we
 * ship as a pure public client with nothing secret to provision.
 *
 * All HTTPS runs through the modern curl (HttpCurl) because device node is 0.9.8k.
 * Box access tokens live ~60 min; refresh tokens ROTATE on every use (~60 days) - the
 * caller MUST persist the NEW refresh_token returned by refresh().
 */
		...
		f.then(this, function () {
			...
			if (!r || r.status !== 200) {
				if (typeof console !== "undefined") {
					console.log("boxnet: token endpoint failed status=" + (r && r.status) +
