/*global OAuth2, GraphApi, console */
 ...
 * Returns credentials in the shape the account DB stores and GraphApi later reads,
 * PLUS a username: accounts/handlers/create.js does Assert.require(args.username) and
 * refuses to create the account without one, so we fetch the Microsoft account name here.
 */
...
ExchangeCodeCommandAssistant.prototype = {
	run: function (future) {
		...
		ex.then(this, function () {
			...
			var info = GraphApi.getAccountInfo(common);
			info.then(this, function () {
				var me = {};
				try { me = info.result || {}; } catch (e2) { me = {}; }
				future.result = {
					returnValue: true,
					username:    me.userPrincipalName || me.mail || "OneDrive",
					alias:       me.displayName || undefined,
