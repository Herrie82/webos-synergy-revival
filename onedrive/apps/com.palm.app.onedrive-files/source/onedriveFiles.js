/*global enyo, window */
/*
 * OnedriveFiles - a minimal OneDrive browser / uploader for the revived connector.
 ...
 *
 * OneDrive (Graph) is ID-based: a "folder" is an opaque item id (root = "root"), NOT a path
 ...
 * Each list entry's `path` field IS its OneDrive id (folder id to descend / file id to fetch).
 *
...
enyo.kind({
	...
	accountId: null,
	account:   null,
	path:      "",        // current OneDrive folder id ("" -> service defaults to root)
	stack:     null,      // ancestor folder ids, for "up a folder" (OneDrive has no parent ptr)

	components: [
		{ kind: "Toolbar", className: "enyo-toolbar-light", pack: "center", components: [
			{ name: "header", kind: "Control", content: "OneDrive" }
		]},
		{ name: "pathLabel", className: "od-path", content: "/" },
		{ name: "status",    className: "od-status", content: "Loading…" },
		{ kind: "Scroller", flex: 1, components: [
			{ name: "list" }
		]},
		{ kind: "Toolbar", className: "od-uploadbar", components: [
		...
		{ name: "dbx",   kind: "PalmService", service: "palm://com.palm.service.onedrive/" }
	],
	...
	create: function () {
		...
		this.log("onedrivefiles: create -> listAccounts");
	...
	gotAccounts: function (s, r) {
		...
		this.log("onedrivefiles: gotAccounts n=" + accts.length + " templates=" +
			accts.map(function (a) { return a.templateId; }).join(","));
		for (var i = 0; i < accts.length; i++) {
			if (accts[i].templateId === "com.palm.onedrive") { this.account = accts[i]; break; }
		}
		if (!this.account) {
			this.setStatus("No OneDrive account. Add one in Settings → Accounts, then reopen.");
			return;
		}
		this.accountId = this.account._id;
		this.$.header.setContent("OneDrive — " + (this.account.username || ""));
		this.browse("");   // "" -> OneDrive root
	},

	// --- browsing ------------------------------------------------------------
	// Load a OneDrive folder by id. Does NOT touch the ancestor stack - descend()/upTapped
	...
	gotList: function (s, r) {
		...
		this.log("onedrivefiles: gotList entries=" + entries.length + " path=" + (this.path || "/"));
	...
	renderList: function (entries) {
		...
		if (this.path) {
			this._rows.push(this.$.list.createComponent(
				{ className: "od-row od-up", content: "↑ up a folder", onclick: "upTapped" },
				{ owner: this }));
		}
		entries.forEach(function (e) {
			var isDir = (e.type === "folder");
			var row = self.$.list.createComponent({
				kind: "HFlexBox", className: "od-row", align: "center", onclick: "rowTapped",
				components: [
					{ flex: 1, className: "od-name", content: e.name + (isDir ? "/" : "") },
					{ className: "od-meta", content: isDir ? "" : self.fmtSize(e.size) }
				]
			}, { owner: self });
			row.entry = e;
			self._rows.push(row);
	...
	uploadTapped: function () {
		...
		// OneDrive upload targets a folder ID (current folder; "" -> service root) with an
	...
	svcFail: function (s, r) {
		...
		this.log("onedrivefiles: svcFail " + code);
