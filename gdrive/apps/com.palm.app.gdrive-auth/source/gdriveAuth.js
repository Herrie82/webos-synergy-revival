...
/*
 * GdriveAuth - customUI account validator for the Google Drive Synergy connector.
 *
 ...
 * Google Drive OAuth consent page (no TLS 1.2/1.3, modern JS). So this app does NO
...
enyo.kind({
	name: "GdriveService",
	kind: "PalmService",
	service: "palm://com.palm.service.gdrive/"
});
...
enyo.kind({
	...
	redirectPrefix: "http://localhost/gdrive/oauth2callback",
	oauthResultKey: "x_gdrive_oauth_result",

	components: [
		{ kind: "Toolbar", className: "enyo-toolbar-light accounts-header", pack: "center", components: [
			{ kind: "Control", name: "title", content: "Google Drive" }
		]},
		{ name: "svc",        kind: "GdriveService", onFailure: "svcFailure" },
		...
		{ kind: "Scroller", flex: 1, components: [
			{ className: "box-center", components: [
				{ name: "status", className: "accounts-body-text", style: "padding:16px; line-height:1.4;",
					content: "Tap Sign In to open the Google Drive sign-in page. After you approve access, you'll be returned here automatically." }
			]}
		]},
	...
	create: function () {
		...
		this.log("gdrive-auth: launch params " + enyo.json.stringify(this.params));
	},
	...
	performSignIn: function () {
		...
		this.setStatus("Opening Google Drive sign-in…");
		this.$.svc.call({ state: "gdrive" }, {
	...
	launchAtlas: function (inSender, inResponse) {
		...
		// Google Drive redirect is never captured and the pref stays empty (=> "credentials
		// incorrect"). Close any running Atlas first, then launch fresh.
		this.log("gdrive-auth: probing for a running Atlas to close before launch");
	...
	atlasClosedThenLaunch: function (inSender, resp) {
		...
		for (var i = 0; i < apps.length; i++) {
			if (apps[i].id === "org.webosports.app.atlas" && apps[i].processid) {
				this.log("gdrive-auth: closing stale Atlas pid " + apps[i].processid);
	...
	doLaunchAtlas: function () {
		if (this.done) { return; }
		this.log("gdrive-auth: launching Atlas simple-mode (cold)");
		this.$.appLaunch.call({
	...
	finish: function (result) {
		this.done = true;
		this.stopPoll();
		this.log("gdrive-auth: sending result " + enyo.json.stringify(result));
