...
var OAuth2 = {
	...
	buildAuthorizeUrl: function (state) {
		...
		return Config.AUTHORIZE_URL +
			...
			// Google-specific: needed to receive a refresh_token. prompt=consent forces it to
			// be re-issued even on a repeat authorization (else it comes back only the first time).
			"&access_type=offline" +
			"&prompt=consent" +
			"&code_challenge="         + encodeURIComponent(challenge) +
			"&code_challenge_method=S256" +
			"&state="                  + encodeURIComponent(state || "gdrive");
	},
		...
		f.then(this, function () {
			...
			if (!r || r.status !== 200) {
				if (typeof console !== "undefined") {
					console.log("gdrive: token endpoint failed status=" + (r && r.status) +
