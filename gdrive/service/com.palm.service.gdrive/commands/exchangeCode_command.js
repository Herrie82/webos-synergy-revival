/*global OAuth2, DriveApi, console */
 ...
 * Returns credentials in the shape the account DB stores and DriveApi later reads,
 * PLUS a username: accounts/handlers/create.js does Assert.require(args.username) and
 * refuses to create the account without one, so we fetch the Google account email here.
 */
...
ExchangeCodeCommandAssistant.prototype = {
	run: function (future) {
		...
		ex.then(this, function () {
			...
			var info = DriveApi.getAccountInfo(common);
			info.then(this, function () {
				var u = {};
				try { u = (info.result && info.result.user) || {}; } catch (e2) { u = {}; }
				future.result = {
					returnValue: true,
					username:    u.emailAddress || "Google Drive",
