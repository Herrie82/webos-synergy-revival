	...
	getUnassociatedChatThreads: function(imArray, phoneArray) {
		...
		var findChatThreadsForAddress = function(type, address) {
			...
			if (type === "phone") {
				...
				// webOS: WhatsApp chatthreads are keyed on the number WITHOUT the leading "+"
				// (Messaging.Utils.normalizeAddress strips it for type_whatsapp), but a contact's
				// WhatsApp im can store its normalizedValue WITH the "+" (e.g. an aggregate person
				// merged with a CardDAV/Google contact whose im normalizedValue is "+31612345678").
				// Align the two so this unassociated-thread lookup actually matches — otherwise the
				// thread is never linked and the conversation shows the bare number instead of the name.
				if (address.type === "type_whatsapp" && normalizedAddress && normalizedAddress.charAt(0) === "+") {
